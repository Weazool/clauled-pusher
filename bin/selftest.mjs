...
// happens to have a token configured - which is how "quota5h is -1 with no
...
// by Sonnet 5 - the raw API id, same shape buildModel()'s transcript fallback
...
check('the model is carried on its own', r.sent?.model === 'Opus 5', r.sent?.model);
check('effort is a top-level field, spelled out', r.sent?.effort === 'medium', r.sent?.effort);
check('header carries the session', r.sent?.session === 'proj', r.sent?.session);
check('quota5h omitted when no quota source', r.sent?.quota5h === undefined, JSON.stringify(r.sent?.quota5h));
check('context labelled ctx', r.sent?.context?.label === 'ctx');
check('context pct from transcript ~70', Math.abs((r.sent?.context?.pct ?? 0) - 70) < 0.5, String(r.sent?.context?.pct));
check('context carries its own token detail', r.sent?.context?.detail === '700k/1M', r.sent?.context?.detail);
const ctxLine = `${r.sent?.context?.label} ${r.sent?.context?.detail} 100%`;
check('context line fits 21 chars at 100%', ctxLine.length <= 21, `${ctxLine.length} - "${ctxLine}"`);
check('the footer object is gone entirely', r.sent?.footer === undefined, JSON.stringify(r.sent?.footer));
...
check('quota5h labelled 5h', r.sent?.quota5h?.label === '5h', r.sent?.quota5h?.label);
check('quota5h comes from rate_limits', r.sent?.quota5h?.pct === 26, String(r.sent?.quota5h?.pct));
check('quota5h carries its own reset countdown', /^(\d+h\d+m|\d+m|now)$/.test(r.sent?.quota5h?.reset ?? ''), r.sent?.quota5h?.reset);
...
// quota7d (the weekly reading) rides the same rate_limits block as quota5h.
// The FIELD says 7d - its source period, what every upstream name calls it -
// while the LABEL says "1w", which is what fits on the glass.
check('quota7d labelled 1w on the glass', r.sent?.quota7d?.label === '1w', r.sent?.quota7d?.label);
check('quota7d comes from seven_day', r.sent?.quota7d?.pct === 21, String(r.sent?.quota7d?.pct));
check('quota7d carries its own reset countdown', /^(\d+h\d+m|\d+m|now)$/.test(r.sent?.quota7d?.reset ?? ''), r.sent?.quota7d?.reset);
...
const quotaLine = `${r.sent?.quota5h?.label} ${r.sent?.quota5h?.reset} 100%`;
check('quota line fits 21 chars at 100%', quotaLine.length <= 21, `${quotaLine.length} - "${quotaLine}"`);
const weekLine = `${r.sent?.quota7d?.label} ${r.sent?.quota7d?.reset} 100%`;
...
check('context from context_window ~3.7', Math.abs((r.sent?.context?.pct ?? 0) - 3.7) < 0.15, String(r.sent?.context?.pct));
...
check('context omitted, not blanked', r.sent?.context === undefined, JSON.stringify(r.sent?.context));
check('quota5h still served from the cache', r.sent?.quota5h?.pct === 26, String(r.sent?.quota5h?.pct));

console.log('\nstatusline degrades safely');
r = await run('statusline.mjs', [], JSON.stringify({ ...payload, transcript_path: '/nonexistent' }));
check('context omitted with no transcript', r.sent?.context === undefined, JSON.stringify(r.sent?.context));
...
// the quota and the context were never sent by a hook at all
// before this.
...
check('busy carries the model from cache', r.sent?.model === 'Opus 5', r.sent?.model);
...
check('busy carries the effort fresh from the payload', r.sent?.effort === 'high', r.sent?.effort);
check('busy carries the cached quota', r.sent?.quota5h?.pct === 26, String(r.sent?.quota5h?.pct));
check('busy carries context from the transcript', Math.abs((r.sent?.context?.pct ?? 0) - 70) < 0.5, String(r.sent?.context?.pct));
...
check('a different session with no cache of its own gets no model', r.sent?.model === undefined, JSON.stringify(r.sent?.model));
...
check('the second session caches its own, different model', r.sent?.model === 'Sonnet 5', r.sent?.model);

r = await run('busy.mjs', ['tool'], JSON.stringify({
  session_id: 'aaaa1111-bbbb-2222-cccc-333344445555',   // back to the first session
  tool_name: 'Bash',
}));
check('the first session still recovers ITS OWN model, unaffected by the second', r.sent?.model === 'Opus 5', r.sent?.model);
...
// ever carries d.model - which would otherwise leave a session's model
...
check('model recovered from the transcript', r.sent?.model === 'Sonnet 5', r.sent?.model);

r = await run('busy.mjs', ['tool'], JSON.stringify({
  session_id: 'ffff7777-0000-1111-2222-333344445555',
  tool_name: 'Read',
}));
check('and is now cached, so a later push with no transcript still has it', r.sent?.model === 'Sonnet 5', r.sent?.model);
...
// further and switching models mid-session left the device naming the old
...
check('cache primed with the first model', r.sent?.model === 'Haiku 4.5', r.sent?.model);
...
check('the switch is picked up from the transcript', r.sent?.model === 'Sonnet 5', r.sent?.model);

r = await run('busy.mjs', ['tool'], JSON.stringify({ session_id: SWITCHED, tool_name: 'Read' }));
check('and the refreshed model is what gets cached', r.sent?.model === 'Sonnet 5', r.sent?.model);
...
check('a live payload model still beats the transcript', r.sent?.model === 'Opus 5', r.sent?.model);
...
check('a proposed plan raises "Review plan"', r.sent?.banner === 'Review plan', JSON.stringify(r.sent?.banner));
...
check('a question raises "Answer question"', r.sent?.banner === 'Answer question', JSON.stringify(r.sent?.banner));
...
check('the plan banner still carries the full display', r.sent?.quota5h?.pct === 26 && r.sent?.effort === 'high', JSON.stringify({ g1: r.sent?.quota5h?.pct, effort: r.sent?.effort }));
check('the plan banner fits one screen line', (r.sent?.banner ?? '').length <= 21);

console.log('\nbanners');
r = await run('event.mjs', ['stop'], '{}');
check('stop raises a banner', r.sent?.banner === 'Your turn', r.sent?.banner);
check('stop clears the spinner', r.sent?.busy === '', JSON.stringify(r.sent?.busy));

// Same regression as busy.mjs: event.mjs used to send only the context, so a Stop
...
check('stop also carries the cached quota', r.sent?.quota5h?.pct === 26, String(r.sent?.quota5h?.pct));
check('stop carries the effort fresh from the payload', r.sent?.effort === 'low', r.sent?.effort);

r = await run('event.mjs', ['notification'], JSON.stringify({ message: 'Permission needed for Bash' }));
const noteText = r.sent?.banner ?? '';
...
check('notification without a message has a default', r.sent?.banner === 'Claude needs input', r.sent?.banner);

console.log('\nno v3 field name survives anywhere');
// The device speaks v4 ONLY - a v3 name is not ignored, it is a field that
// silently never renders. Checked against all THREE push shapes, because
// busy.mjs and event.mjs spread buildDisplay() and then add their own keys:
// that seam is exactly where a half-migrated field would survive a rename
// that looked complete in buildDisplay() alone.
const DEAD = ['title', 'gauge1', 'gauge2', 'gauge3', 'row', 'footer', 'events'];
const full = JSON.stringify({ ...payload, rate_limits: { five_hour: { used_percentage: 26, resets_at: soon }, seven_day: { used_percentage: 21, resets_at: soon + 86_400 } } });
for (const [label, script, args] of [
  ['statusline', 'statusline.mjs', []],
  ['busy',       'busy.mjs',       ['tool']],
  ['event',      'event.mjs',      ['stop']],
]) {
  r = await run(script, args, full);
  const survivors = DEAD.filter((k) => r.sent?.[k] !== undefined);
  check(`${label} push carries no v3 field`, survivors.length === 0, survivors.join(',') || 'clean');
  check(`${label} push declares v4`, r.sent?.v === 4, String(r.sent?.v));
}
